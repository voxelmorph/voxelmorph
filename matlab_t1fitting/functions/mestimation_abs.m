function [pmap, sdmap, null_index, S, areamask] = mestimation_abs(data_struct, configs, pmap_mse, null_index, S)
...
%                 attention: mask air
                if (max(dvec) < 10) || ( ~areamask(tx, ty) ) % mask air voxels
                    continue;
                end
...
%                 dweight( approxd < 0 ) = max( - approxd( approxd < 0), 1.0) ;
                ...
                elseif strcmpi(configs.type, 'cauchy')
                    if SD < 50
                        ...
                        pres = fminsearch( @(px) sum( ...
                            cauchyloss( (abs(dvec(:)) - abs(MOLLI3param(px, tvec_sorted(:) )) ) ,  10) , ...
                            'all'), [150, 2, 1e3], options);  
                    end
                ...
                elseif strcmpi(configs.type, 'geman')
                    if SD < 50
                        ...
                        pres = fminsearch( @(px) sum( ...
                            gemanloss( (abs(dvec(:)) - abs(MOLLI3param(px, tvec_sorted(:)) )),  10) , ...
                            'all'), [150, 2, 1e3], options);
                    end
                ...
                elseif strcmpi(configs.type, 'welsch')
                    if SD < 50
                        ...
                        pres = fminsearch( @(px) sum( ...
                            welschloss( (abs(dvec(:)) - abs(MOLLI3param(px, tvec_sorted(:)) )) ,  10) , ...
                            'all'), [150, 2, 1e3], options);
                    end
                else
